...
export function WebhookRow({
  ...
  function handleWebhookChange(enabled: boolean) {
    if (enabled && !webhookExists) {
      createWebhookMutation.mutate(
        ...
        {
          onSuccess: () => {
            notifySuccess('Success', t('docker.container.webhookCreated'));
            onSuccess();
          },
        }
      );
      ...
      deleteWebhookMutation.mutate(
        ...
        {
          onSuccess: () => {
            notifySuccess('Success', t('docker.container.webhookDeleted'));
            onSuccess();
          },
        }
      );
  ...
  return (
    <tr>
      <td>
        <SwitchField
          label={t('docker.container.containerWebhook')}
