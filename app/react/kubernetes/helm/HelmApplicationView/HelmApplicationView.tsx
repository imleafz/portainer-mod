...
export function HelmApplicationView() {
  ...
  return (
    <>
      <PageHeader
        title={t('kubernetes.helm.title')}
        breadcrumbs={[
          {
            label: t('kubernetes.helm.applications'),
...
function HelmDetails({
  ...
  if (isError) {
    return <Alert color="error" title={t('kubernetes.helm.failedToLoad')} />;
  }

  if (!release || !selectedRevision) {
    return <Alert color="error" title={t('kubernetes.helm.notFound')} />;
  }
